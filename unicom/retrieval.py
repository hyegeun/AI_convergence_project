...
def main():
    ...
    if args.eval:
        ...
        new_state_dict = {key.replace('model.', ''): value for key, value in model.state_dict().items()}
        print(new_state_dict.keys())
        # 수정된 상태 사전을 저장
        torch.save(new_state_dict, f'./output/sop_2.pt')
...
@torch.no_grad()
def euclidean_distance(x, y, topk=10):
...
def get_metric(
    ...
    if gallery is None:
        ...
        while not is_end:
            ...
            dists, indices_pt = euclidean_distance(query[idx:end], gallery)
            indices_np = indices_pt.cpu().numpy()
            list_pred_top1.append(indices_np[:, 0])
            list_pred_top10.append(indices_np[:, :10])

            idx += 128

        query_label = np.array(query_label).reshape(num_feat)
        pred_top1 = np.concatenate(list_pred_top1).reshape(num_feat)
        pred_top10 = np.concatenate(list_pred_top10).reshape(num_feat, 10)

        make_dir('result')
        for q_pid, g_pids in zip(query_label, gallery_label[pred_top10]):
            query_path = f'result/{q_pid:04d}'
            make_dir(query_path)

            original_path = f'./unicom/data/aiproject/test_dir'

            for idx, g_pid in enumerate(g_pids):
                img = f'{g_pid}.jpg'
                shutil.copy(f'{original_path}/{img}', f'{query_path}/{idx:03d}.jpg')


        rank_1 = np.sum(query_label == gallery_label[pred_top1]) / num_feat

        rank_10 = 0
        for i in range(num_feat):
            rank_10 += np.any(query_label[i] == gallery_label[pred_top10[i]])

        rank_1 = float(rank_1)
        rank_10 = float(rank_10) / num_feat

        return [rank_1 * 100, rank_10 * 100]
